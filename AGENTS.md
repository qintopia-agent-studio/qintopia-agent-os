...
- As of 2026-07-15, the corrected `v0.2.10` same-SHA follow-up deploy installed the new
  ...
  `restart_targets` is rejected before promotion and does not trigger rollback. Before
  reuse, the existing tree must satisfy the same deploy-runner owner, non-writable,
  directory accessibility, regular/symlink type, sidecar `0755`, and metadata `0444`
  checks as a new staging tree. Invalid existing trees fail closed, and same-SHA reuse
  must preserve a distinct `previous` target.
