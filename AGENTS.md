...
- Xiaoman daily case-report JPEG rendering must stay storyline/character-first in both
  ...
  production hosts without Playwright do not regress to a statistics-first poster. The
  production auto-publish worker must call the renderer with `--json-summary-only`; it
  may consume only `character_universe_summary`, `public_output_style`, private-review
  counts, artifact identity, and paths. Do not parse or forward full
  `daily_report_markdown`, full `character_universe`, quote-map, wiki, draft,
  run-manifest, or operator-review text in the send chain.
