...
2026-09-24 本地切片在已配置的范围增加岸岸运营确认门禁。UI 与可信群入口共用一条 WorkItem，可在同一明确提交中先确认人员关联，再确认具体版本的统一资料/卡片；工作账号按真实主体归责。后续
`decision`
在准备及发送前重验运营确认，再执行原有本栋内容/发布规则。没有运营配置的既有案例沿用原路径，不自动修改现存授权。

本栋欢迎词仍按舍长有效安排；运营确认不等于发布许可。真实企微交互映射、群投递、人员匹配来源、阿靓正式制卡及完整试点验收仍独立待接，测试回执不代表真实送达。
