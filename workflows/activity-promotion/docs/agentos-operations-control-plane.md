...
| Xiaoman send request starter timer 尚未线上部署验收   | 线上 approved poster brief 是否会自动补 awaiting_publish 群发请求还未确认 | 本地 `scripts/server-deploy.sh prepare/deploy` 已安装并启用 `qintopia-agentos-xiaoman-activity-send-request-starter-worker.timer`；它只创建 awaiting_publish `group_message_request` work item，不记录最终确认、不排队、不发送、不调用飞书/企微/外部 adapter                                         |
