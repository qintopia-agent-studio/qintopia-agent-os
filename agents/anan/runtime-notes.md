...
每次返回绑定 call、task、agent、operation、工具名及插件版本，记录原始输入、输出和插件源码的 SHA256。宿主核对绑定与哈希后记录持久回执。子进程不继承数据库或 token 环境，不提供网络、子进程或直接外发工具。该入口处理已受信任宿主生成的合成任务，不是任意调用方的授权接口。
