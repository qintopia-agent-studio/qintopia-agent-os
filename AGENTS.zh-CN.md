...
- PR 就绪检查：`pnpm pr:doctor`
- PR body 验证：`pnpm pr:check-body`
- PR 创建：`pnpm pr:create -- --body-file <completed-pr-body.md>`
...
- 不要把预填的 GitHub compare 链接交给人作为默认 PR 流程。先运行
  `pnpm pr:doctor`，再用完整 PR body 运行 `pnpm pr:create`。如果缺少 GitHub CLI，先运行
  `pnpm pr:bootstrap` 并按提示完成 `gh auth login`。
...
- 用 `pnpm pr:check-body` 或 `pnpm pr:doctor` 验证完整 PR body。
...
8. 产品范围改动先读 `docs/product/agent-os-prd.md`
9. Agent OS 设计改动先读 `docs/agent-os/README.md`
10. 历史迁移证据读 `docs/plans/completed/monorepo-migration.md`
11. 目标 package README 或 manifest
12. `docs/engineering/` 或 `docs/operations/` 下相关文档
