...
if [[ "${QINTOPIA_ERHUA_LEGACY_CRON_RETIREMENT:-}" != "approved-production-erhua-legacy-cron-retirement" ]]; then
  echo "Erhua legacy cron retirement requires explicit owner approval" >&2
  exit 1
fi
...
EXPECTED_PREVIOUS_SHA256="f55862d5289224a089d32dea712baf6fcb6a6ff1fbeb7da0cbe6131aacb3cef9"
...
retirement_script = "retire-erhua-legacy-cron-production.sh"
...
if entry_stat.st_mode & 0o022:
    fail("legacy cron file must not be group/world writable")
...
try:
    ...
    os.chmod(temp_name, stat.S_IMODE(entry_stat.st_mode))
...
echo "Erhua legacy cron retirement passed"
