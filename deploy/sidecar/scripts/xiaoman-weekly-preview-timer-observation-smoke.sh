...
if [[ "${QINTOPIA_XIAOMAN_WEEKLY_PREVIEW_TIMER_OBSERVATION_ENABLE:-}" != "1" ]]; then
  echo "xiaoman weekly preview timer observation skipped: set QINTOPIA_XIAOMAN_WEEKLY_PREVIEW_TIMER_OBSERVATION_ENABLE=1" >&2
  exit 0
fi

EXPECTED_STATE="${QINTOPIA_XIAOMAN_WEEKLY_PREVIEW_TIMER_EXPECTED_STATE:-enabled}"
EXPECTED_CALENDAR="${QINTOPIA_XIAOMAN_WEEKLY_PREVIEW_TIMER_CALENDAR:-Mon *-*-* 09:30:00}"
...
if [[ "$EXPECTED_STATE" != "enabled" && "$EXPECTED_STATE" != "disabled" ]]; then
  echo "xiaoman weekly preview observation expected state must be enabled or disabled" >&2
  exit 1
fi
if [[ ! -f "$ENV_FILE" ]]; then
...
if [[ "$EXPECTED_STATE" == "enabled" ]]; then
  ...
  if [[ -z "$next_elapse" || "$next_elapse" == "0" || "$next_elapse" == "infinity" || "$next_elapse" == "n/a" ]]; then
...
echo "xiaoman weekly preview timer observation passed"
