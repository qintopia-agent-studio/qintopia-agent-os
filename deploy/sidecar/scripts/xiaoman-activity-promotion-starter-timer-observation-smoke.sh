...
if [[ "${QINTOPIA_XIAOMAN_ACTIVITY_PROMOTION_STARTER_TIMER_OBSERVATION_ENABLE:-}" != "1" ]]; then
  echo "xiaoman activity promotion starter timer observation skipped: set QINTOPIA_XIAOMAN_ACTIVITY_PROMOTION_STARTER_TIMER_OBSERVATION_ENABLE=1 to inspect runtime timer state" >&2
  exit 0
fi
...
SERVICE_NAME="${QINTOPIA_XIAOMAN_ACTIVITY_PROMOTION_STARTER_SERVICE_NAME:-qintopia-agentos-xiaoman-activity-promotion-starter-worker.service}"
TIMER_NAME="${QINTOPIA_XIAOMAN_ACTIVITY_PROMOTION_STARTER_TIMER_NAME:-qintopia-agentos-xiaoman-activity-promotion-starter-worker.timer}"
EXPECTED_INTERVAL="${QINTOPIA_XIAOMAN_ACTIVITY_PROMOTION_STARTER_TIMER_INTERVAL_EXPECTED:-2min}"
JOURNAL_LINES="${QINTOPIA_XIAOMAN_ACTIVITY_PROMOTION_STARTER_TIMER_JOURNAL_LINES:-80}"
...
if ! command -v "$SYSTEMCTL" >/dev/null 2>&1; then
  echo "systemctl is required for Xiaoman activity promotion starter timer observation" >&2
  exit 1
fi

if ! command -v "$JOURNALCTL" >/dev/null 2>&1; then
  echo "journalctl is required for Xiaoman activity promotion starter timer observation" >&2
  exit 1
fi
...
grep -E "ExecStart=.*run-xiaoman-activity-promotion-starter-worker --once --apply$" "$service_unit" >/dev/null
assert_no_sensitive_output "service unit" "$service_unit"

timer_unit="$tmp_dir/timer-unit.txt"
"$SYSTEMCTL" cat "$TIMER_NAME" >"$timer_unit"
grep -F "OnBootSec=6min" "$timer_unit" >/dev/null
...
"${BIN_CMD[@]}" run-xiaoman-activity-promotion-starter-worker --check-only >"$worker_check"
...
assert payload["worker"] == "xiaoman-activity-promotion-starter-worker"
...
echo "xiaoman activity promotion starter timer observation passed"
