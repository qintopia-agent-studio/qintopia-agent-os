...
if [[ "${QINTOPIA_XIAOMAN_WEEKLY_RECRUITMENT_PRODUCTION_ACTIVATION:-}" != "approved-production-xiaoman-weekly-recruitment" ]]; then
  echo "xiaoman weekly recruitment production activation requires explicit owner approval" >&2
  exit 1
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OBSERVATION_SCRIPT="${SCRIPT_DIR}/xiaoman-weekly-recruitment-production-observation-smoke.sh"
...
if [[ ! -x "$OBSERVATION_SCRIPT" || ! -x "$LEGACY_CRON_OBSERVATION_SCRIPT" ]]; then
  echo "xiaoman weekly recruitment activation requires release-local observation scripts" >&2
  exit 1
fi
if [[ ! -f "$ENV_FILE" ]]; then
  echo "xiaoman weekly recruitment activation requires the persistent sidecar env file" >&2
  exit 1
fi
if ! command -v "$SYSTEMCTL" >/dev/null 2>&1; then
  echo "systemctl is required for xiaoman weekly recruitment activation" >&2
  exit 1
fi
...
require_env_line() {
  ...
  if [[ "$count" != "1" ]]; then
    echo "xiaoman weekly recruitment activation requires exactly one ${key}" >&2
    exit 1
  fi
  if ! grep -Fxq "${key}=${expected}" "$ENV_FILE"; then
    echo "xiaoman weekly recruitment activation requires ${key}=${expected}" >&2
    exit 1
  fi
}
...
if ! grep -F "ExecStart=/usr/bin/env QINTOPIA_DEPLOYED_COMMIT_SHA=${EXPECTED_RELEASE_SHA} " "$service_unit" >/dev/null; then
  echo "xiaoman weekly recruitment activation requires service unit bound to ${EXPECTED_RELEASE_SHA}" >&2
  cleanup_failed_activation
  exit 1
fi
...
if ! env -i \
  ...
  QINTOPIA_XIAOMAN_WEEKLY_RECRUITMENT_PRODUCTION_RELEASE_SHA="$EXPECTED_RELEASE_SHA" \
  "$OBSERVATION_SCRIPT" >/dev/null; then
  cleanup_failed_activation
  exit 1
fi

echo "xiaoman weekly recruitment production timer enabled"
