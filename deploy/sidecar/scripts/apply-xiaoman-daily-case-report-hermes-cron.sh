...
if [[ "${QINTOPIA_XIAOMAN_DAILY_CASE_REPORT_HERMES_CRON:-}" != "approved-production-xiaoman-daily-case-report-hermes-cron" ]]; then
  echo "xiaoman daily case report Hermes cron apply requires explicit owner approval" >&2
  exit 1
fi

MODE="${1:---install}"
if [[ "$MODE" != "--install" && "$MODE" != "--enable" ]]; then
  echo "usage: apply-xiaoman-daily-case-report-hermes-cron.sh [--install|--enable]" >&2
  exit 2
fi
...
WRAPPER_SOURCE="${RELEASE_CURRENT}/runtime/hermes/scripts/qintopia_xiaoman_daily_case_report.sh"
...
def write_cron(document: dict, previous_payload: bytes, entry_stat: os.stat_result) -> dict:
    ...
    backup_path = cron_file.with_name(
        f"jobs.json.daily-case-report-{stamp}-{previous_sha256[:12]}.bak"
    )
...
def find_reviewed_job(jobs: list[dict]) -> tuple[int, dict]:
    ...
    if len(matches) != 1:
        fail("Hermes cron file must contain exactly one reviewed daily case report job")
    index, job = matches[0]
...
if mode == "--install":
    ...
    if len(matching_name) > 1:
        fail("Hermes cron file contains duplicate daily case report jobs")
    if any(job.get("script") == JOB_SCRIPT and job.get("name") != JOB_NAME for job in jobs):
        fail("Hermes cron file already declares a job bound to the daily case report script")
    ...
    if matching_name:
        ...
        status = "daily_case_report_hermes_cron_already_installed"
        ...
        evidence = write_cron(document, previous_payload, entry_stat)
        status = "daily_case_report_hermes_cron_installed"
        job_enabled = False
    report = {
        "schema_version": 1,
        "status": status,
    ...
    if job.get("enabled") is True and not schema_normalized:
        ...
        report = {
            "schema_version": 1,
            "status": "daily_case_report_hermes_cron_enabled"
            if enabled_changed
            else "daily_case_report_hermes_cron_already_enabled",
            "profile": "xiaoman",
            "job_enabled": True,
            "wrapper_installed": False,
            "origin_chat_id_resolved": True,
...
if [[ "$snapshot_sync_ok" != "true" ]]; then
  echo "xiaoman daily case report Hermes cron snapshot sync did not run; run it manually" >&2
fi
echo "xiaoman_daily_case_report_hermes_cron_snapshot_sync_ok=${snapshot_sync_ok}"
echo "xiaoman daily case report Hermes cron apply passed"
