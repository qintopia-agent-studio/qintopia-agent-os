...
if [[ "${QINTOPIA_XIAOMAN_WEEKLY_PREVIEW_HERMES_CRON:-}" != "approved-production-xiaoman-weekly-preview-hermes-cron" ]]; then
  echo "xiaoman weekly preview Hermes cron apply requires explicit owner approval" >&2
  exit 1
fi

MODE="${1:---install}"
if [[ "$MODE" != "--install" && "$MODE" != "--enable" ]]; then
  echo "usage: apply-xiaoman-weekly-preview-hermes-cron.sh [--install|--enable]" >&2
  exit 2
fi
...
if [[ ! -x "$PYTHON_BIN" ]]; then
  echo "qintopia_hermes_cron_apply_safe_failure=xiaoman weekly preview Hermes cron apply requires fixed python3" >&2
  echo "xiaoman weekly preview Hermes cron apply requires fixed python3" >&2
  exit 1
fi
...
JOB_NAME = "小满·周一活动预告（文字稿+海报简报）"
JOB_SCHEDULE_EXPR = "30 9 * * 1"
JOB_SCRIPT = "qintopia_xiaoman_weekly_preview.sh"
STATUS_INSTALLED = "weekly_preview_hermes_cron_installed"
STATUS_ALREADY_INSTALLED = "weekly_preview_hermes_cron_already_installed"
...
def fail(message: str) -> None:
    raise SystemExit(
        f"qintopia_hermes_cron_apply_safe_failure=xiaoman weekly preview Hermes cron apply failed: {message}"
    )
...
def write_cron(document: dict, previous_payload: bytes, entry_stat: os.stat_result) -> dict:
    ...
    backup_path = cron_file.with_name(
        f"jobs.json.weekly-preview-{stamp}-{previous_sha256[:12]}.bak"
    )
...
def find_reviewed_job(jobs: list[dict]) -> tuple[int, dict]:
    ...
    if len(matches) != 1:
        fail("Hermes cron file must contain exactly one reviewed weekly preview job")
    index, job = matches[0]
    schedule = job.get("schedule")
    if not isinstance(schedule, dict) or schedule.get("expr") != JOB_SCHEDULE_EXPR:
        fail("reviewed weekly preview job schedule does not match the reviewed declaration")
    if job.get("script") != JOB_SCRIPT or job.get("no_agent") is not True:
        fail("reviewed weekly preview job script contract does not match the declaration")
    if job.get("deliver") != "origin":
        fail("reviewed weekly preview job deliver mode does not match the reviewed declaration")
    origin = job.get("origin")
    if not isinstance(origin, dict) or origin.get("platform") != "wecom":
        fail("reviewed weekly preview job origin platform does not match the reviewed declaration")
    if origin.get("chat_name") is not None or origin.get("thread_id") is not None:
        fail("reviewed weekly preview job origin routing fields do not match the reviewed declaration")
    if origin.get("chat_id") != resolve_chat_id():
        fail("reviewed weekly preview job origin chat id drifted from the Xiaoman profile env")
    return index, job
...
if mode == "--install":
    ...
    if len(matching_name) > 1:
        fail("Hermes cron file contains duplicate weekly preview jobs")
    ...
    if job.get("enabled") is True and not schema_normalized:
        report = {
            "schema_version": 1,
            "status": "weekly_preview_hermes_cron_already_enabled",
            "profile": "xiaoman",
            "job_enabled": True,
        ...
        report = {
            "schema_version": 1,
            "status": "weekly_preview_hermes_cron_enabled"
            if enabled_changed
            else "weekly_preview_hermes_cron_already_enabled",
            "profile": "xiaoman",
            "job_enabled": True,
            "wrapper_installed": False,
            "origin_chat_id_resolved": True,
...
if [[ "$snapshot_sync_ok" != "true" ]]; then
  echo "xiaoman weekly preview Hermes cron snapshot sync did not run; run it manually" >&2
fi
echo "xiaoman_weekly_preview_hermes_cron_snapshot_sync_ok=${snapshot_sync_ok}"
echo "xiaoman weekly preview Hermes cron apply passed"
