...
if [[ "${QINTOPIA_XIAOMAN_WEEKLY_PLAN_CONFIRMATION_HERMES_CRON:-}" != "approved-production-xiaoman-weekly-plan-confirmation-hermes-cron" ]]; then
  echo "Xiaoman weekly plan confirmation Hermes cron apply requires explicit owner approval" >&2
  exit 1
fi

if [[ "${1:-}" != "--install" && "${1:-}" != "--enable" ]]; then
  echo "usage: apply-xiaoman-weekly-plan-confirmation-hermes-cron.sh --install|--enable" >&2
  exit 2
fi
...
WRAPPER_SOURCE="${RELEASE_DIR}/runtime/hermes/scripts/qintopia_xiaoman_weekly_plan_confirmation.sh"
...
[[ -f "$CRON_FILE" ]] || fail "Xiaoman cron jobs.json is missing"
[[ -f "$PROFILE_ENV" ]] || fail "Xiaoman profile env is missing"
...
mode = sys.argv[3]
job_name = "小满·周日活动计划确认"
job_schedule = {"kind": "cron", "expr": "0 20 * * 0", "display": "0 20 * * 0"}
job_script = "qintopia_xiaoman_weekly_plan_confirmation.sh"
...
def parse_chat_id(path: Path) -> str:
    ...
    if stat.S_ISLNK(env_stat.st_mode) or not stat.S_ISREG(env_stat.st_mode):
        fail("Xiaoman profile env must be a regular non-symlink file")
    mode_bits = stat.S_IMODE(env_stat.st_mode)
    if mode_bits & (stat.S_IWGRP | stat.S_IWOTH):
        fail("Xiaoman profile env must not be group/world writable")
    if env_stat.st_size <= 0 or env_stat.st_size > 1024 * 1024:
        fail("Xiaoman profile env size is invalid")
    ...
    for raw in text.splitlines():
        ...
        if key != "WECOM_HOME_CHANNEL":
            continue
        if key in values:
            fail("Xiaoman profile env contains duplicate WECOM_HOME_CHANNEL")
        ...
        try:
            ...
            fail("Xiaoman profile env contains unsafe WECOM_HOME_CHANNEL quoting")
        if len(parts) != 1:
            fail("Xiaoman profile env contains unsafe WECOM_HOME_CHANNEL")
        value = parts[0]
        if not re.fullmatch(r"[A-Za-z0-9_@.\-]{1,128}", value):
            fail("Xiaoman profile env WECOM_HOME_CHANNEL is not a supported chat id")
        values[key] = value
    if "WECOM_HOME_CHANNEL" not in values:
        fail("Xiaoman profile env is missing WECOM_HOME_CHANNEL")
...
if stat.S_ISLNK(parent_stat.st_mode) or not stat.S_ISDIR(parent_stat.st_mode):
    fail("Xiaoman cron parent must be a regular directory")
...
if previous_mode & 0o002:
    fail("Xiaoman cron file must not be world writable")
...
if "schema_version" in value and value.get("schema_version") != 1:
    fail("Xiaoman cron jobs.json schema_version must be 1")
jobs = value["jobs"]
...
if len(existing) > 1:
    fail("Xiaoman cron jobs.json contains duplicate weekly plan confirmation jobs")
...
if mode == "--install":
    if existing:
        if not job_def_matches(existing[0]):
            fail("existing weekly plan confirmation job definition drifts from the reviewed declaration")
    else:
        jobs.append(
            {
                "id": os.urandom(6).hex(),
    ...
    if not existing:
        fail("cannot enable weekly plan confirmation job before it is installed")
    if not job_def_matches(existing[0]):
        fail("existing weekly plan confirmation job definition drifts from the reviewed declaration")
...
print(
    json.dumps(
        {
            "schema_version": 1,
            "status": "weekly_plan_confirmation_hermes_cron_applied",
            "mode": mode,
            "profile": "xiaoman",
            "job_name": job_name,
