...
KEY="QINTOPIA_XIAOMAN_DAILY_CASE_REPORT_AUTO_PUBLISH_PRODUCTION_APPROVAL"

fail() {
  local reason="$1"
  echo "qintopia_runtime_one_shot_safe_failure=xiaoman daily case report approval repair: ${reason}" >&2
  echo "xiaoman daily case report approval repair failed: ${reason}" >&2
  exit 1
}

if [[ "${QINTOPIA_XIAOMAN_DAILY_CASE_REPORT_APPROVAL_REPAIR:-}" != "$APPROVAL" ]]; then
  fail "approval missing"
fi

expected_release_sha="${QINTOPIA_XIAOMAN_DAILY_CASE_REPORT_APPROVAL_REPAIR_RELEASE_SHA:-}"
...
python3 - "$ENV_FILE" "$KEY" "$PUBLISH_APPROVAL" <<'PY' || fail "persistent env update failed"
...
if len(matches) > 1:
    raise SystemExit("approval key is duplicated")
if len(matches) == 1:
    if matches[0] != line:
        raise SystemExit("approval key value invalid")
    print("xiaoman_daily_case_report_approval_repair=deduped")
    raise SystemExit(0)
...
print("xiaoman_daily_case_report_approval_repair=applied")
PY

echo "xiaoman daily case report approval repair completed"
