...
if [[ "${QINTOPIA_XIAOMAN_ACTIVITY_SIGNAL_TIMER_OBSERVATION_ENABLE:-}" != "1" ]]; then
  echo "xiaoman activity signal timer observation skipped: set QINTOPIA_XIAOMAN_ACTIVITY_SIGNAL_TIMER_OBSERVATION_ENABLE=1 to inspect runtime timer state" >&2
  exit 0
fi
...
SERVICE_NAME="${QINTOPIA_XIAOMAN_ACTIVITY_SIGNAL_SERVICE_NAME:-qintopia-agentos-xiaoman-activity-signal-worker.service}"
TIMER_NAME="${QINTOPIA_XIAOMAN_ACTIVITY_SIGNAL_TIMER_NAME:-qintopia-agentos-xiaoman-activity-signal-worker.timer}"
EXPECTED_INTERVAL="${QINTOPIA_XIAOMAN_ACTIVITY_SIGNAL_TIMER_INTERVAL_EXPECTED:-2min}"
JOURNAL_LINES="${QINTOPIA_XIAOMAN_ACTIVITY_SIGNAL_TIMER_JOURNAL_LINES:-80}"
...
if ! command -v "$SYSTEMCTL" >/dev/null 2>&1; then
  echo "systemctl is required for Xiaoman activity signal timer observation" >&2
  exit 1
fi

if ! command -v "$JOURNALCTL" >/dev/null 2>&1; then
  echo "journalctl is required for Xiaoman activity signal timer observation" >&2
  exit 1
fi
...
grep -E "ExecStart=.*run-xiaoman-activity-signal-worker --once --apply$" "$service_unit" >/dev/null
assert_no_sensitive_output "service unit" "$service_unit"

timer_unit="$tmp_dir/timer-unit.txt"
"$SYSTEMCTL" cat "$TIMER_NAME" >"$timer_unit"
grep -F "OnBootSec=5min" "$timer_unit" >/dev/null
...
"${BIN_CMD[@]}" run-xiaoman-activity-signal-worker --check-only >"$worker_check"
...
assert payload["worker"] == "xiaoman-activity-signal-worker"
...
assert isinstance(payload["scanned_count"], int)
assert isinstance(payload["work_items"], list)
PY
assert_no_sensitive_output "worker check" "$worker_check"

echo "xiaoman activity signal timer observation passed"
