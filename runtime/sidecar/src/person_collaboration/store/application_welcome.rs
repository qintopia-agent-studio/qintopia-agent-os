...
impl Store {
    pub(crate) async fn application_reconcile_welcome(
        ...
        for case in cases {
            ...
            let bound = sqlx::query("UPDATE qintopia_agent_os.work_items SET payload=$2,metadata=metadata || '{\"local_only\":true,\"application_link_from_store\":true}'::jsonb WHERE id=$1 AND work_item_type='welcome_event' AND requester_agent='anan' AND target_agent='anan' AND (payload='{}'::jsonb OR payload=$2)")
                .bind(work).bind(payload).execute(&mut *tx).await?;
            ensure!(
