#!/usr/bin/env bash
# Hermes no_agent cron wrapper for the Xiaoman Monday weekly preview.
# Instantiated from runtime/hermes/scripts/qintopia-hermes-cron-wrapper.template.sh with
# TASK_NAME=xiaoman-weekly-preview and WORKER_SCRIPT=xiaoman-weekly-preview-worker.sh.
...
WORKER="${RELEASE_CURRENT}/deploy/sidecar/scripts/xiaoman-weekly-preview-worker.sh"
...
# the mere definition, so clear the group to keep every Hermes run identical to the
# release-managed timer run.
unset QINTOPIA_RELEASE_DIR
unset QINTOPIA_XIAOMAN_WRAPPER_PATH
unset QINTOPIA_XIAOMAN_WEEKLY_PREVIEW_PYTHON
unset QINTOPIA_XIAOMAN_WEEKLY_PREVIEW_OUTPUT_DIR
...
export QINTOPIA_XIAOMAN_WEEKLY_PREVIEW_ENABLED=1

# The retired systemd unit bound the release SHA at the exec boundary; derive the same
# value from release/current so downstream release-identity checks behave identically.
