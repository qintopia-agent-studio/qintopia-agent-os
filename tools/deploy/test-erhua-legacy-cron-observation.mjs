...
const tmpRoot = fs.mkdtempSync(path.join(tmpParent, "qintopia-erhua-cron-"));
...
try {
  ...
  if (invalid.status === 0 || !invalid.stderr.includes("must be JSON")) {
    throw new Error("Erhua legacy cron observation accepted invalid JSON");
  }

  const outside = run({
    QINTOPIA_ERHUA_LEGACY_CRON_FILE: path.join(tmpRoot, "jobs.json"),
  });
  if (outside.status === 0) {
    throw new Error(
      "Erhua legacy cron observation accepted a cron file outside profile"
    );
  }
} finally {
...
console.log("Erhua legacy cron observation test passed.");
