...
const tmpRoot = fs.mkdtempSync(
  path.join(tmpParent, "qintopia-xiaoman-weekly-preview-hermes-cron-")
);
...
const sourceWrapper = path.join(
  repoRoot,
  "runtime/hermes/scripts/qintopia_xiaoman_weekly_preview.sh"
);
...
const sourceDeclaration = path.join(
  repoRoot,
  "runtime/hermes/cron/xiaoman/weekly-preview.job.json"
);
...
const approval = "approved-production-xiaoman-weekly-preview-hermes-cron";

const jobName = "小满·周一活动预告（文字稿+海报简报）";
const jobExpr = "30 9 * * 1";
const jobScript = "qintopia_xiaoman_weekly_preview.sh";
...
const listBackups = (cronDir) =>
  fs
    .readdirSync(cronDir)
    .filter((name) => name.startsWith("jobs.json.weekly-preview-"))
    .sort();
...
try {
  ...
  const applyScript = path.join(tmpRoot, "apply-xiaoman-weekly-preview-hermes-cron.sh");
  ...
  check(
    !applySource.includes("QINTOPIA_XIAOMAN_WEEKLY_PREVIEW_HERMES_CRON_FILE") &&
  ...
  const runApproved = (args) =>
    run(args, { QINTOPIA_XIAOMAN_WEEKLY_PREVIEW_HERMES_CRON: approval });
  ...
  check(
    ...
    "declaration template drifted from the reviewed weekly preview contract"
  );
  ...
  check(
    ...
    "reviewed registry entry drifted from the weekly preview declaration"
  );
  ...
  check(
    result.stdout.includes('"status":"weekly_preview_hermes_cron_installed"') &&
      ...
      result.stdout.includes(
        "xiaoman_weekly_preview_hermes_cron_snapshot_sync_ok=false"
      ),
  ...
  result = runApproved(["--install"]);
  check(result.status !== 0, "install accepted a duplicate weekly preview job");
  check(
    result.stderr.includes("already declares the weekly preview job") &&
      sha256(fs.readFileSync(cronFile)) === baselineSha &&
  ...
  check(
    result.stdout.includes('"status":"weekly_preview_hermes_cron_enabled"') &&
  ...
  check(
    result.stdout.includes('"status":"weekly_preview_hermes_cron_already_enabled"') &&
  ...
  check(
    result.status !== 0 &&
      result.stderr.includes("exactly one reviewed weekly preview job"),
  ...
  const writeFakeWorker = (exitCode) => {
    fs.writeFileSync(
      ...
      [
        ...
        'echo "PREVIEW_PYTHON_DEFINED=${QINTOPIA_XIAOMAN_WEEKLY_PREVIEW_PYTHON+yes}"',
        'echo "PREVIEW_OUTPUT_DIR_DEFINED=${QINTOPIA_XIAOMAN_WEEKLY_PREVIEW_OUTPUT_DIR+yes}"',
        `echo "${draftBody}"`,
        `exit ${exitCode}`,
        "",
      ].join("\n"),
  ...
  const wrapperScript = path.join(tmpRoot, "qintopia_xiaoman_weekly_preview.sh");
  fs.writeFileSync(
    wrapperScript,
    fs
  ...
  const wrapperLog = path.join(
    wrapperStateRoot,
    "xiaoman-weekly-preview/hermes-cron.log"
  );
  const runWrapper = () =>
    spawnSync("bash", [wrapperScript], {
      ...
      env: {
        ...
        QINTOPIA_XIAOMAN_WEEKLY_PREVIEW_OUTPUT_DIR: "/evil/output",
      },
      encoding: "utf8",
    });
  ...
  check(
    logText.includes(draftBody) &&
      ...
      logText.includes("PREVIEW_PYTHON_DEFINED=\n") &&
      logText.includes("PREVIEW_OUTPUT_DIR_DEFINED=\n"),
  ...
  check(
    result.stdout.trim() ===
      "xiaoman-weekly-preview worker failed (exit=3); evidence in server-local log",
...
console.log("Xiaoman weekly preview Hermes cron apply test passed.");
