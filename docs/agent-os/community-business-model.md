...
合入远端主线 `6be9ac1`，本页核对的本地源码 `0806c2e`
尚未同步。接线时复用合并后的成果，不重放旧登录分支或复制第二套账号系统。账号登录已有权限校验，但不自动证明 QiWe 自然人映射或二花业务授权已接通。
...
| 生活线索与体验 | 附近有条适合散步的路线、某家店体验不错 | 关联地点或资源，保留分享时间、主观评价和来源；事实未核实不写成社区保证，过时或矛盾信息可更正 |
| 社区意见与需求 | 希望厨房添一个电饭煲、晚上有点吵       | 保存为反馈并归并相关主题；表达意见不自动变成投诉、已证实问题或已批准采购                     |
...
社区意见先按主题、范围及问题进展聚合，区分重复转述与不同人的反馈。需要业务决定或实际处理时，才交对应舍长、主管或智能体推进；结果回到原事项。回复反馈人或群内公布仍按适用联系与发布约定执行，不能承诺系统尚未完成的处理。
...
UI 是同一数据的工作视图：设施页看当前状态，意见页看主题和跟进，生活指南看适用的地点与经验，知识页看正式规则。保留必要来源和历史供查看、更正；不强制展示每条提取结果，也不生成全量待审清单。对话更正与 UI 更正使用同一服务。
...
先复用 C 的身份、范围、记忆修订与受控读取，再交付一条附近推荐和一条意见跟进的本地闭环；完整前台采集、全部 UI 和全领域导入不追加为 D 欢迎流程的前置条件。这些场景均为待实施验收，不是本轮已运行的测试。
