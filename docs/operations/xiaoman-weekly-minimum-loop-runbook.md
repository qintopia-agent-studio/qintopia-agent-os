...
The durable timer path for the Saturday recruitment is now
`docs/operations/xiaoman-weekly-recruitment-hermes-cron-runbook.md`; the Sunday
plan-confirmation still uses `docs/operations/xiaoman-weekly-loop-cutover-runbook.md`,
and the recruitment section of that loop runbook is kept only as the rollback target.
The confirmed next-week preview is governed by its own Hermes cron runbook. Do not
hot-edit production units or recreate the Hermes cron after cutover.
