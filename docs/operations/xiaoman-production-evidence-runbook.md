...
change services. The current Release SHA is validated through `release/current`; Feishu
delivery services receive their release identity from release-rendered systemd units,
not from persistent env drift. The persistent Huabaosi Feishu primary-storage approval
remains `approved-huabaosi-feishu-artifact-mirror`; do not replace it with the separate
