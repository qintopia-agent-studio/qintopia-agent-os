# Xiaoman Weekly Preview Hermes Cron Runbook

Updated: 2026-08-10

This runbook moves the Monday 09:30 Xiaoman weekly activity preview timer from the
release-managed systemd timer back to a Xiaoman Hermes cron job. Behavior does not
change: the job reads the next Monday-Sunday activity window, writes the operator-review
draft and the weekly poster brief to server-local state, and sends nothing to a group.
The poster brief rides the same worker; "poster goes straight to the group" is a
separate later task and stays out of scope here.
...
- `runtime/hermes/scripts/qintopia_xiaoman_weekly_preview.sh` - the `no_agent` wrapper
- `runtime/hermes/cron/xiaoman/weekly-preview.job.json` - sanitized declaration template
- `runtime/hermes/cron/reviewed-cron-jobs.json` - reviewed allowlist entry
- `deploy/sidecar/scripts/apply-xiaoman-weekly-preview-hermes-cron.sh` - the only writer
...
- `deploy/sidecar/scripts/apply-xiaoman-weekly-preview-production-config.sh` - env gate
- `deploy/sidecar/scripts/rollback-xiaoman-weekly-preview-production.sh` - timer stop
...
The worker `xiaoman-weekly-preview-worker.sh` skips with exit 0 unless
`QINTOPIA_XIAOMAN_WEEKLY_PREVIEW_ENABLED=1`, while
`rollback-xiaoman-weekly-preview-production.sh` expects the persistent sidecar env file
to carry `QINTOPIA_XIAOMAN_WEEKLY_PREVIEW_ENABLED=0` before it stops the systemd timer.
Those two requirements cannot both be met from the env file after cutover, so the Hermes
wrapper exports `QINTOPIA_XIAOMAN_WEEKLY_PREVIEW_ENABLED=1` itself.

End state after cutover:

- `/etc/qintopia/message-sidecar.env` keeps `QINTOPIA_XIAOMAN_WEEKLY_PREVIEW_ENABLED=0`,
  which keeps the retired systemd path inert.
...
3. The current systemd timer is still the live producer. Record the last systemd run for
   the comparison in step 2 (the 2026-08-10 09:30 run is the reference):
   ...
   passes only while the Hermes preview job is absent or already reviewed; after this
   task lands, the `reviewed-cron-jobs.json` registry entry covers it, so the allowlist
   smoke in step 5 is the ongoing health signal.
...
QINTOPIA_XIAOMAN_WEEKLY_PREVIEW_HERMES_CRON=approved-production-xiaoman-weekly-preview-hermes-cron \
  /home/ubuntu/qintopia-agent-os-releases/current/deploy/sidecar/scripts/apply-xiaoman-weekly-preview-hermes-cron.sh --install
```
...
A disabled job never fires, so this step is safe at any time.
...
sudo -n cat \
  /home/ubuntu/.local/state/qintopia-agentos/xiaoman-weekly-preview/latest-summary.json
sudo -n tail -n 5 \
  /home/ubuntu/.local/state/qintopia-agentos/xiaoman-weekly-preview/hermes-cron.log
```

The summary must keep the same shape as the systemd run: an operations-review draft of
preview work items plus the `latest-weekly-poster-brief.json` review artifact, no
external send. Compare structure and work-item types, not exact text (activity titles
may have shifted). The log line must read `run=ok`.

The config apply and activation scripts
(`apply-xiaoman-weekly-preview-production-config.sh`,
`activate-xiaoman-weekly-preview-production.sh`) stay valid for the persistent env
values the worker still consumes; the wrapper sources the same env file, so no config
re-apply is needed during cutover.
...
The config script sets `QINTOPIA_XIAOMAN_WEEKLY_PREVIEW_ENABLED=0` and keeps the three
other reviewed values, then the rollback script disables and stops only
`qintopia-agentos-xiaoman-weekly-preview.timer` and its service.
...
```bash
QINTOPIA_XIAOMAN_WEEKLY_PREVIEW_HERMES_CRON=approved-production-xiaoman-weekly-preview-hermes-cron \
  /home/ubuntu/qintopia-agent-os-releases/current/deploy/sidecar/scripts/apply-xiaoman-weekly-preview-hermes-cron.sh --enable
```
...
`weekly_preview_hermes_cron_already_enabled` and writes nothing.
...
`reviewed_decl_count` including this job. The weekly preview smoke must report the
...
The next natural fire is Monday 2026-08-17 09:30 Asia/Shanghai. Complete the cutover
...
Disable the Hermes job first, then restore the systemd timer only if the preview is
...
paths would run the same worker on Monday morning.
...
- The job still produces the operations-review draft and the weekly poster brief, and
  performs no external send; the poster brief remains a review artifact only.
