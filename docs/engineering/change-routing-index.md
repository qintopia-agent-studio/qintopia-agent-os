...
| Change Erhua morning brief                  | `workflows/erhua-morning-brief/README.md`, `workflows/erhua-morning-brief/workflow.yaml`                                                                                     | `workflows/erhua-morning-brief/morning_brief.py`, `skills/qintopia-tools/variants/xiaoman/`, QunMind `daily-report --public-only`, `runtime/sidecar/src/operations.rs`, `deploy/sidecar/scripts/*erhua-morning-brief*` | `pnpm workflows:check`, `python3 -m unittest discover -s workflows/erhua-morning-brief/tests -v`, `node tools/deploy/check-deploy-contracts.mjs` |
...
| Change scheduled jobs or cron-like behavior | `agents/<agent>/runtime-notes.md`, `runtime/hermes/README.md`, `workflows/silaoshi-daily-ops/README.md`                                                                      | `runtime/sidecar/src/daily_digest_publisher.rs`, `runtime/sidecar/config/agentos/`, `deploy/sidecar/scripts/render-systemd-units.sh`, `runtime/systemd/README.md`                             | `pnpm runtime:hermes:check`, `pnpm deploy:systemd:check`, `pnpm deploy:contracts:check`, `pnpm check:runtime`         |
...
| Change Xiaoman daily case report            | `workflows/xiaoman-daily-case-report/README.md`, `workflows/xiaoman-daily-case-report/workflow.yaml`, `docs/plans/active/xiaoman-daily-case-report-auto-publish.md`          | `workflows/xiaoman-daily-case-report/daily_case_report.py`, `runtime/sidecar/src/operations.rs`, `runtime/sidecar/src/qiwe_image_send_state.rs`, `deploy/sidecar/scripts/render-systemd-units.sh` | `pnpm workflows:check`, `python3 -m unittest discover -s workflows/xiaoman-daily-case-report/tests`, `node tools/deploy/check-deploy-contracts.mjs` |
...
| Change Huabaosi WeCom gateway migration     | `agents/huabaosi/README.md`, `docs/plans/active/huabaosi-wecom-migration.md`, `runtime/hermes/README.md`                                                                     | `runtime/sidecar/`, `runtime/nginx/`, `deploy/sidecar/scripts/`, `deploy/restart-target-rules.yaml`, `docs/reports/2026-07-14-huabaosi-wecom-busy-ack-leak.md`                                | Start docs-only with Markdown lint; later PRs add `pnpm check:runtime`, deploy smokes, and owner-approved canary      |
...
| Add a new workflow                          | `workflows/_template/workflow.yaml`, `docs/engineering/package-contract.md`                                                                                                  | `registry/workflows.yaml`, `deploy/restart-target-rules.yaml`, target `workflows/<workflow>/`, replay fixtures under `fixtures/`                                                              | `pnpm workflows:check`, `pnpm registry:check`, restart impact preview                                                 |
