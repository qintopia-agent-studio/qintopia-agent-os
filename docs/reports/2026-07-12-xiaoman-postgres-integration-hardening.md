...
| A workbench review changed an artifact to `changes_requested` before a later fixture reused it for a send request. | Smoke ordering defect   | Keep the integrated review fixture `approved`; the separate unit tests cover `changes_requested`. | The apply smoke now preserves the approved-artifact precondition through the send path. |
