...
提交 `7573d1c` 的主 CI `36141657126`
已通过 Light、Runtime、Rust、PostgreSQL 与聚合检查；固定隔离库中的 apply
smoke 步骤也通过。

独立 business `36141615799` 失败于私有申请配置正例：模拟文件默认放在 Linux `/tmp`
下，父目录为公共可写，生产校验按契约拒绝。一次性 Linux 容器已复现同一失败，原日志为
`application-linux-temp-reproduced.log`。

修复仅调整包内模拟目录，复用凭据场景已有的仓库忽略目录模式，并新增公共可写父目录必须拒绝的反例。不修改生产加载器、CI 或共享框架。后续提交仍须核对自己的 CI，不能沿用
`7573d1c` 的通过结论。

修复后在一次性 Linux 容器中降为 UID 1001，PMS 包 92 项全部通过，见
`application-linux-nonroot-verified.log`。该非 root 证据限于包级文件权限与单元场景，生产 Gateway 运行用户和安装条件仍须另行验收。

该 head 的建议审阅提出签名与请求路径可能不同。核对 `resident_welcome/protocol.rs:15` 与
`production_events_tests.rs`：两处均为
`/api/v1/ingress/pms/events`，真实 TCP 场景也已通过，故记录为无需改代码；不将两个独立期待值改成互相引用的同一表达式。自动审阅明确排除了大部分实现文件，只能作为部分建议审阅，不表示全面批准或已关闭机器人意见。
