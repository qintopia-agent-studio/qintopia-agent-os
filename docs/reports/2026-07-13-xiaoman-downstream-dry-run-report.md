...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- | -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `run-evidence-worker --once --dry-run` and `run-collaboration-worker --work-item-type visual_asset_request --once --dry-run` returned `apply_requested=false` but incorrectly reported `dry_run=false`. | Read-only report-contract defect | Derive each worker report's `dry_run` value from `apply_requested` in one place. | Unit tests assert fixture dry-run reports have `dry_run=true` and `apply_requested=false`; the production preflight retains its fail-closed assertion. |
