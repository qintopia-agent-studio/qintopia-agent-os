...
- activation 先确认持久化 mirror flag 唯一且为
...
或含 secret 临时文件。parser 在校验 flag value 前忽略其他 env
key，因此数据库 URL、token 或 secret 中的标点不会影响 observation。observation 只检查 unit/timer 状态和非 secret
mirror observation
