...
launcher 在 disabled state 只传递该 flag 和 non-secret release SHA；enabled
state 通过固定 systemd preflight 和 read-only observation
unit 使用生产 EnvironmentFile，shell 不读取或传递 secret，也不使用 `source`、`eval`
