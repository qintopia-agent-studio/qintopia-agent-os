...
Status: draft, owner decision required before implementation Scope: 阿靓（画报司 /
`huabaosi`）受控图片生成，不含飞书写回、企微发送或对外发布
...
未确认这些项时，worker 只能返回 `adapter_not_configured` 或 preview，不得尝试网络调用。
