...
Status: request intake merged on `master`; historical provider identified; external
adapter owner decision required
...
4. 需要发送时，仍须经过现有 group-message final
   confirmation 与 send-ready 边界。当前 send-request starter 仍引用 approved
   `poster_brief`；真实图片 adapter 完成后，需单独评审并实现下游改为只引用 approved
   `generated_image`。
