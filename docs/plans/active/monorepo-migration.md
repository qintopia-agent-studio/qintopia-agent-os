...
- Continued M10-B by importing the reviewed server command into the monorepo:
  - copied `/home/ubuntu/.hermes/scripts/qintopia-collab-mcp` into
  ...
  - added `pnpm mcp:collab:check` and wired it into `pnpm check:light`
...
- Completed M10-C production repoint:
  - CI passed for `e7227734f6e84493f7709290d615297daf8d1165`
  - published an opt-in deploy bundle for `e7227734f6e84493f7709290d615297daf8d1165`
    without rebuilding the sidecar runtime artifact
  - verified the deploy bundle from COS on the server, including `SHA256SUMS`,
    `artifact-manifest.json`, `qintopia-agent-os-deploy-bundle.tar.gz`, and the
    `skills/qintopia-tools/variants/{erhua,xiaoman,wenyuange}` payload
  - confirmed the deploy bundle did not include
    `skills/qintopia-tools/docs/server-backups` evidence files
  - assembled immutable release directory
    `/home/ubuntu/qintopia-agent-os-releases/e7227734f6e84493f7709290d615297daf8d1165`
    by preserving the existing sidecar runtime from
    `13a3957369ad80ea8b6e93d4c67c6ef120ecffd6` and overlaying the new deploy bundle
    payload
  - set `/home/ubuntu/qintopia-agent-os-releases/previous` to
    `f1b5f4e44066bcf4aa6621738c4c15c2a107df2a` and `current` to
    `e7227734f6e84493f7709290d615297daf8d1165`
  - backed up the old profile-local `qintopia-tools` plugin directories under
    `/home/ubuntu/qintopia-agent-os-backups/m10c-qintopia-tools-20260705T142000Z`
  - repointed Wenyuange, Xiaoman, and Erhua one at a time so
    `.hermes/profiles/<profile>/plugins/qintopia-tools` is now a symlink to
    `/home/ubuntu/qintopia-agent-os-releases/current/skills/qintopia-tools/variants/<profile>`
  - verified all three profile services are active, all three plugin variants import
    successfully through the profile plugin path, the release sidecar `check` passes
    against NATS and Postgres, and all nine Qintopia system services are active
  - did not delete or archive the old profile-local plugin copies; cleanup remains
    M11/M12 gated
- Completed M10-D production repoint:
  - reconciled Erhua `qiwe-platform` before release packaging; server tracked source is
    `main@6f69794` with only the historical untracked backup
    `adapter.py.bak.home-group-send-20260607-1050`, which remains server-side audit
    evidence and was not packaged
  - changed `skills/qiwe/adapter.py` so the fallback context MCP command points through
    `/home/ubuntu/qintopia-agent-os-releases/current` instead of the deprecated
    `qintopia-msg-sidecar` checkout or the diagnostic server git checkout
  - added `skills/qiwe` to the deploy bundle payload and extended deploy preflight to
    require the package and reject deprecated QiWe context MCP fallback paths
  - CI passed for `99681909149fde4f16daa3af941a750d1f239860`
  - published an opt-in deploy bundle for `99681909149fde4f16daa3af941a750d1f239860`
    without rebuilding the sidecar runtime artifact
  - verified the deploy bundle from COS on the server, including `SHA256SUMS`,
    `artifact-manifest.json`, `qintopia-agent-os-deploy-bundle.tar.gz`, and the
    `skills/qiwe` payload
  - assembled immutable release directory
    ...
    payload
  - set `/home/ubuntu/qintopia-agent-os-releases/previous` to
    `e7227734f6e84493f7709290d615297daf8d1165` and `current` to
    `99681909149fde4f16daa3af941a750d1f239860`
  - backed up the old Erhua profile-local `qiwe-platform` plugin checkout under
    `/home/ubuntu/qintopia-agent-os-backups/m10d-qiwe-platform-20260705T144000Z`
  - repointed `.hermes/profiles/erhua/plugins/qiwe-platform` to
    `/home/ubuntu/qintopia-agent-os-releases/current/skills/qiwe`
  - verified Erhua, Xiaoman, Wenyuange, Huabaosi, and Silaoshi profile services are
    active, all nine Qintopia system services are active, the release sidecar `check`
    passes against NATS and Postgres, there are no running process refs to
    `qintopia-msg-sidecar`, the old profile-local QiWe plugin path, or the diagnostic
    monorepo checkout, and the release-managed QiWe plugin contains no `pycache`, `pyc`,
    or backup files
...
- Completed M10-E production repoint:
  - CI passed for `16496c8d4bfb13ed26d080727a4c812f9c2e0487`
  - published an opt-in deploy bundle for `16496c8d4bfb13ed26d080727a4c812f9c2e0487`
    without rebuilding the sidecar runtime artifact
  - verified the deploy bundle from COS on the server, including `SHA256SUMS`,
    `artifact-manifest.json`, `qintopia-agent-os-deploy-bundle.tar.gz`, and the
    `skills/feishu-base` payload
  - migrated the four Base app/table identifiers out of the old plugin source into the
    Huabaosi runtime `.env`; Feishu app credentials were already present in that runtime
    env
  - assembled immutable release directory
    `/home/ubuntu/qintopia-agent-os-releases/16496c8d4bfb13ed26d080727a4c812f9c2e0487`
    by preserving the existing sidecar runtime from the previous release and overlaying
    the new deploy bundle payload
  - set `/home/ubuntu/qintopia-agent-os-releases/previous` to
    `99681909149fde4f16daa3af941a750d1f239860` and `current` to
    `16496c8d4bfb13ed26d080727a4c812f9c2e0487`
  - backed up the old Huabaosi profile-local `qintopia-base-read` plugin and pre-change
    `.env` under
    `/home/ubuntu/qintopia-agent-os-backups/m10e-qintopia-base-read-20260705T151129Z`
  - repointed `.hermes/profiles/huabaosi/plugins/qintopia-base-read` to
    `/home/ubuntu/qintopia-agent-os-releases/current/skills/feishu-base`
  - verified Huabaosi service active state, release-managed plugin import and tool
    registration, required runtime env presence after restart, release sidecar `check`,
    all nine Qintopia system services, and five active Hermes profile services
  - removed Python cache files generated by the import smoke from the release-managed
...
- Started M12 legacy cleanup with the owner-approved low-risk archive batch:
  - added `docs/operations/archive-readiness/m12-low-risk-archive.md`
  - archived the M11 low-risk candidates under
    `/home/ubuntu/qintopia-agent-os-backups/m12-low-risk-20260706T011023Z`
  - moved `/home/ubuntu/qintopia-msg-sidecar`, `/home/ubuntu/qintopia-agent-os`,
    `/home/ubuntu/qintopia-hermes-runtime`,
    `/home/ubuntu/qintopia-message-sidecar-build`, `/home/ubuntu/qintopia-artifacts`,
    `/home/ubuntu/qintopia-migration`, `/home/ubuntu/worktool-gateway-old`, and the
    existing `/home/ubuntu/qintopia-worklog-guard-*` directories into that archive
  - did not permanently delete any files
  - kept `/home/ubuntu/worktool-gateway`, `/home/ubuntu/.hermes/profiles/xiaoqin`, and
    `/opt/qiwe-openclaw-adapter` in place for separate decommission batches
  - verified `current` and `previous` release symlinks were unchanged
  - verified all nine `qintopia-message-*` and `qintopia-agentos-*` system services were
...
5. For each remaining decommission batch, remove or disable references first, verify no
   process, unit, timer, cron, MCP command, or nginx route references the path, then
   archive first with owner approval. Permanent deletion remains a later approval step.
