...
def _window_reason(items: list[dict[str, Any]], start: str, end: str, predicate, default: str) -> str:
    weather_texts = []
    max_pop = 0
    max_precip = 0.0
    for item in items:
...
def _annotate_windows(windows: list[dict[str, str]], items: list[dict[str, Any]], predicate, default_reason: str) -> list[dict[str, str]]:
    annotated = []
        ...
        warnings.append(
            {
                ...
                "title": _clean_text(item.get("headline") or item.get("description"), max_len=160),
                ...
                "start_time": _clean_text(item.get("effectiveTime") or item.get("issuedTime"), max_len=40),
...
def _qweather_payload(args: dict[str, Any], bundle: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    umbrella_windows = _time_windows(umbrella_source, umbrella_predicate, "fxTime", max_windows=8)
    ...
    umbrella_windows = _annotate_windows(umbrella_windows, umbrella_source, umbrella_predicate, umbrella_reason)

    thunderstorm_source = hourly[: _qintopia_weather_horizon(args)]
    thunderstorm_windows = _time_windows(thunderstorm_source, _qweather_thunder_hour, "fxTime")
    thunderstorm_windows = _annotate_windows(thunderstorm_windows, thunderstorm_source, _qweather_thunder_hour, "小时预报有雷暴信号")
    ...
    if "weather_alert" in errors:
        limitations.append("QWeather Weather Alert data unavailable; do not claim no official warning")
