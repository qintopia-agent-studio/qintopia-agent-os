...
runtime that should let Hermes directly receive sanitized read results from the sidecar.
Read-through is limited to read-only, non-dry-run operations and returns the worker's
`record_count`, `records`, and `summaries`; write operations still return commands.
